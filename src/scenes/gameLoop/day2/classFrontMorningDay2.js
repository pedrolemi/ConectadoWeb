...
export default class ClassFrontMorningDay2 extends ClassFrontBase {
    ...
    create(params) {
        ...
        if (!this.gameManager.getValue(this.gameManager.isLate)) {  
            let nodes = this.cache.json.get('classFrontMorningDay2');
            this.tablesNode = super.readNodes(nodes, "day2\\classFrontMorningDay2", "table", true);
        }
        // Si no, se colocan mas alumnos en la clase y se pone directamente el nodo del profesor
            ...
            let nodes = this.cache.json.get('classFrontMorningDay2');
            let teacherNode = super.readNodes(nodes, "day2\\classFrontMorningDay2", "teacher", true);
